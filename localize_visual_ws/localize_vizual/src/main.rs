...
pub fn main() {
    ...
    for point in lines.iter() {
        obstacles.push(Object { position: *point,
            id: humans_num,});
        vec.push(Gm::new(
            Circle::new(&context, vec2((point.0 * CM_TO_M*scale) as f32, (point.1 * CM_TO_M*scale) as f32), 25.0*scale as f32),
    ...
    /////////////////////////////////////////
    vec.push(Gm::new(
        Circle::new(&context, vec2((field_x*CM_TO_M*0.5*scale) as f32, (field_y*CM_TO_M*0.5*scale) as f32), 25.0*scale as f32),
    ...
    window.render_loop(move |frame_input: FrameInput| unsafe {
        ...
        for i in 0..humans_num {
            ...
            vec[humans[i].id].set_center(vec2((humans[i].position.0 * CM_TO_M*scale) as f32,
                                              (humans[i].position.1 * CM_TO_M*scale) as f32));
        }
        /////////////////////////////////////////
